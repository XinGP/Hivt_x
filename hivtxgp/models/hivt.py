...
from losses import PolygonLoss
...
class HiVT(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.decoder = MLPDecoder(local_channels=embed_dim,
                                  ...
                                  num_modes=num_modes,
                                  uncertain=True)
        self.future_encoder = FutureEncoder(historical_steps=30,
                                          node_dim=node_dim,
                                          edge_dim=edge_dim,
                                          embed_dim=embed_dim,
                                          num_heads=num_heads,
                                          ...
                                          local_radius=local_radius,
                                          parallel=parallel)
        self.fglobal_interactor = FGlobalInteractor(historical_steps=30,
                                                  embed_dim=embed_dim,
                                                  edge_dim=edge_dim,
                                                  num_modes=num_modes,
                                                  num_heads=num_heads,
                                                  ...
                                                  dropout=dropout,
                                                  rotate=rotate)
        self.fdecoder = FMLPDecoder(local_channels=embed_dim,
                                  global_channels=embed_dim,
                                  future_steps=future_steps,
                                  num_modes=num_modes,
                                  uncertain=True)
    ...
    def forward(self, data: TemporalData):
        ...
        y_hat = self.decoder(local_embed=local_embed, global_embed=global_embed)

        future_embed = self.future_encoder(data=data, y_hat=y_hat)
        fglobal_embed = self.fglobal_interactor(data=data, local_embed=future_embed, y_hat=y_hat)
        fy_hat, fpi = self.fdecoder(future_embed=future_embed, future_global_embed=fglobal_embed, local_embed=local_embed, global_embed=global_embed)

        return y_hat, fy_hat, fpi

    def training_step(self, data, batch_idx):
        y_hat, fy_hat, fpi = self(data)
        # poly = data['polygon']
        # last_y_hat_point = y_hat[:, data['agent_index'], -1, : 2]

        # last_y_hat_point_device = last_y_hat_point.to(device=data.x.device)  # Ensure the same device
        # polygon_loss_fn = PolygonLoss(poly, 1.0)
        # polygon_loss = polygon_loss_fn(last_y_hat_point_device).mean()

        # Add the polygon loss to the total loss
        ...
        reg_loss = self.reg_loss(y_hat[reg_mask], data.y[reg_mask])
        ...
        valid_steps = reg_mask.sum(dim=-1)
        cls_mask = valid_steps > 0
        l2_norm = (torch.norm(fy_hat[:, :, :, : 2] - data.y, p=2, dim=-1) * reg_mask).sum(dim=-1)  # [F, N]
        best_mode = l2_norm.argmin(dim=0)
        y_hat_best_future = fy_hat[best_mode, torch.arange(data.num_nodes)]
        reg_loss_future = self.reg_loss(y_hat_best_future[reg_mask], data.y[reg_mask])
        soft_target = F.softmax(-l2_norm[:, cls_mask] / valid_steps[cls_mask], dim=0).t().detach()
        cls_loss_future = self.cls_loss(fpi[cls_mask], soft_target)
        loss = 0.2 * reg_loss + reg_loss_future + cls_loss_future
        self.log('train_reg_loss', reg_loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=1)
        self.log('reg_loss', reg_loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=1)
        return loss
    ...
    def validation_step(self, data, batch_idx):
        ...
        self.minMR.update(y_hat_agent, y_agent)
