...
from models_future import MultipleInputEmbedding
from models_future import SingleInputEmbedding
...
class FutureEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.historical_steps = 30
        self.parallel = parallel

        self.drop_edge = DistanceDropEdge(local_radius)
        self.aa_encoder = FFEncoder(historical_steps=historical_steps,
                                    node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout,
                                    parallel=parallel)
        self.temporal_encoder = FTemporalEncoder(historical_steps=historical_steps,
                                                embed_dim=embed_dim,
                                                num_heads=num_heads,
                                                dropout=dropout,
                                                num_layers=num_temporal_layers)
        self.al_encoder = FLEncoder(node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)

    def forward(self, data: TemporalData, y_hat: torch.Tensor) -> torch.Tensor:
        # 车车交互编码
        # 未来编码
        future_bos_mask = data['padding_mask'][:, 20:]
        # future_mask[:, : 20] = data['bos_mask']
        positions_future = y_hat[:, :, :2] + data['positions'][:, 19].unsqueeze(-2)
        for t in range(self.historical_steps):
            data[f'edge_index_{t}'], _ = subgraph(subset=~data['padding_mask'][:, t], edge_index=data.edge_index)
            data[f'edge_attr_{t}'] = \
                positions_future[data[f'edge_index_{t}'][0], t] - positions_future[data[f'edge_index_{t}'][1], t]
        if self.parallel:
            snapshots = [None] * self.historical_steps
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                snapshots[t] = Data(x=y_hat[:, t, :2], edge_index=edge_index, edge_attr=edge_attr,
            ...
            out = self.aa_encoder(x=batch.x, t=None, edge_index=batch.edge_index, edge_attr=batch.edge_attr,
                                  bos_mask=future_bos_mask, rotate_mat=data['rotate_mat'])
            out = out.view(self.historical_steps, out.shape[0] // self.historical_steps, -1)
        else:
            out = [None] * self.historical_steps
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                out[t] = self.aa_encoder(x=y_hat[:, t, :2], t=t, edge_index=edge_index, edge_attr=edge_attr,
                                         bos_mask=future_bos_mask[:, t], rotate_mat=data['rotate_mat'])
...
# 智能体之间的交互
class FFEncoder(MessagePassing):
    ...
    def __init__(self,
                 ...
                 parallel: bool = False,
                 **kwargs) -> None:
        super(FFEncoder, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.historical_steps = 30
...
# 时空编码器
class FTemporalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        super(FTemporalEncoder, self).__init__()
        encoder_layer = FTemporalEncoderLayer(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout)
...
# 时空编码器层
class FTemporalEncoderLayer(nn.Module):

    def __init__(self,
                 embed_dim: int,
                 num_heads: int = 8,
                 dropout: float = 0.1) -> None:
        super(FTemporalEncoderLayer, self).__init__()
...
# 智能体和道路的特征交互
class FLEncoder(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(FLEncoder, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
