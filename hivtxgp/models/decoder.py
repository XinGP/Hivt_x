...
class MLPDecoder(nn.Module):
    ...
    def forward(self,
        ...
        loc_1 = self.loc_1(out).view(-1, self.future_steps, 2)  # [F, N, H, 2]
        ...
        if self.uncertain:
            ...
            scale_1 = F.elu_(self.scale_1(out), alpha=1.0).view( -1, self.future_steps, 2) + 1.0
            # scale = scale + self.min_scale  # [F, N, H, 2]
            scale_1 = scale_1 + self.min_scale
            return torch.cat((loc_1, scale_1), dim=-1)  # [F, N, H, 4], [N, F]
        else:
            return loc_1  # [F, N, H, 2], [N, F]
