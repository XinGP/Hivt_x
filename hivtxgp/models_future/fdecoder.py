...
class FMLPDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(FMLPDecoder, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
        ...
        self.aggr_embed = nn.Sequential(
            nn.Linear((self.hidden_size + self.input_size)*2, self.hidden_size),
            nn.LayerNorm(self.hidden_size),
            nn.ReLU(inplace=True))
        self.loc = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size),
    ...
    def forward(self,
                ...
                future_global_embed: torch.Tensor, local_embed: torch.Tensor,
                global_embed: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        pi = self.pi(torch.cat((future_embed.expand(self.num_modes, *local_embed.shape),
                                future_global_embed, local_embed.expand(self.num_modes, *local_embed.shape),
                                global_embed), dim=-1)).squeeze(-1).t()
        out = self.aggr_embed(torch.cat((future_embed.expand(self.num_modes, *local_embed.shape),
                                future_global_embed, local_embed.expand(self.num_modes, *local_embed.shape),
                                global_embed), dim=-1))
...
class localDecoder(nn.Module):
    def __init__(self,
        ...
        self.loc = nn.Sequential(
            ...
            nn.LayerNorm(self.hidden_size),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_size,  30*2))
        if uncertain:
            self.scale = nn.Sequential(
                nn.Linear(self.hidden_size, self.hidden_size),
                nn.LayerNorm(self.hidden_size),
    ...
    def forward(self,
                local_embed: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:

        end = self.aggr_embed(local_embed).view(self.num_modes, local_embed.size(0), 2)
        loc = self.loc(local_embed).view(local_embed.size(0), 30, 2)
        if self.uncertain:
            scale = F.elu_(self.scale(local_embed), alpha=1.0).view(local_embed.size(0), self.future_steps, 2) + 1.0
            scale = scale + self.min_scale  # [F, N, H, 2]
            scale_end = F.elu_(self.scale_end(local_embed), alpha=1.0).view(self.num_modes, local_embed.size(0), 2) + 1.0
            scale_end = scale_end + self.min_scale  # [F, N, H, 2]
            return torch.cat((end, scale_end), dim=-1), torch.cat((loc, scale), dim=-1)
        else:
            return end, loc   # [F, N, 2]
...
class futureDecoder(nn.Module):
    ...
    def forward(self,
                local_embed: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # 将 global_embed 与 local_embed 进行连接
        #combined_embed = torch.cat((local_embed, global_embed), dim=-1)

        pi = self.pi(local_embed).view(local_embed.size(0), self.num_modes)
        out = self.aggr_embed(local_embed).view(self.num_modes, local_embed.size(0), 64)
        loc = self.loc(out).view(self.num_modes, local_embed.size(0), self.future_steps, 2)  # [F, N, H, 2]
        if self.uncertain:
            scale = F.elu_(self.scale(out), alpha=1.0).view(self.num_modes, -1, self.future_steps, 2) + 1.0
            scale = scale + self.min_scale  # [F, N, H, 2]
            return torch.cat((loc, scale), dim=-1), pi  # [F, N, H, 4], [N, F]
