...
from losses import FinalPredictionL1Loss
from losses import LaplaceNLLLoss
from losses import SoftTargetCrossEntropyLoss
from metrics import ADE
from metrics import FDE
from metrics import MR
from models_future import FGlobalInteractor
from models_future import LocalEncoder
from models_future import FMLPDecoder
from models_future import localDecoder
from models_future import futureDecoder
from utils import TemporalData
from models_future import FutureEncoder

class HiVT_future(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 T_max: int,
                 **kwargs) -> None:
        super(HiVT_future, self).__init__()
        ...
        self.local_encoder = LocalEncoder(historical_steps=historical_steps,
                                          ...
                                          parallel=parallel)
        self.future_encoder = FutureEncoder(historical_steps=50,
                                            node_dim=node_dim,
                                            edge_dim=edge_dim,
                                            embed_dim=embed_dim,
                                            num_heads=num_heads,
        ...
        self.decoder = MLPDecoder(local_channels=embed_dim,
                                  ...
                                  num_modes=num_modes,
                                  uncertain=True)
        self.future_decoder = futureDecoder(local_channels=embed_dim,
                                  global_channels=embed_dim,
                                  future_steps=future_steps,
                                  num_modes=num_modes,
                                  uncertain=True)
        self.localdecoder = localDecoder(local_channels=embed_dim,
                                     global_channels=embed_dim,
                                     future_steps=future_steps,
                                     num_modes=num_modes,
                                     uncertain=True)
        ...
        self.end_loss = FinalPredictionL1Loss(reduction='mean')
    ...
    def forward(self, data: TemporalData):
        ...
        future_embed = self.future_encoder(data=data)
        global_embed = self.global_interactor(data=data, local_embed=future_embed)
        y_hat, pi = self.decoder(local_embed=future_embed, global_embed=global_embed)
        # xin
        # 求出不确定性低的车辆的未来轨迹，将不确定性高的未来轨迹进行掩码
        """y_one_mask = y_one.clone().detach()
        y_end = y_local_end.clone().detach()
        pred_std_end = y_end.std(dim=0)
        end_std = torch.sqrt(torch.sum(pred_std_end ** 2, dim=1))
        mask = end_std < 4
        mask_y_hat = torch.zeros_like(y_one_mask)
        mask_y_hat[mask, :, :] = True
        y_one_certain = y_one_mask * mask_y_hat.to(dtype=y_one_mask.dtype)
        # 将不确定性底的轨迹作为已知
        position_xy = torch.zeros(data.num_nodes, 50, 2, device=self.device)
        # data: position_xy以及xy生成
        position_y = y_one_certain + data['positions'][:, 19].unsqueeze(-2)
        position_xy[:, :20, :] = data['positions'][:, :20, :]
        position_xy[:, 20:, :] = position_y[:, :, :]
        xy = torch.cat((data['x'], y_one_certain), dim=1)
        data['position_xy'] = position_xy
        data['xy'] = xy
        future_embed = self.future_encoder(data=data)
        y_hat, pi = self.future_decoder(local_embed=future_embed)"""
        # 未来交互器
